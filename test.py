...
def process_data(file_content):
    ...
    if "Agent name" in header:
        ...
        done_col = 3
        vendor_share_col = 4
        mistakes_percent_col = 6
        mistakes_value_col = 7
        start_data = 1
    ...
    for r in rows[start_data:]:
        ...
        try:
            ...
            mistakes_percent = float(r[mistakes_percent_col]) if r[mistakes_percent_col].strip() != "" else 0
            mistakes_value = float(r[mistakes_value_col]) if r[mistakes_value_col].strip() != "" else 0
        except:
            continue
    ...
    if pot is None:
        raise ValueError("Pot value (F18) not found in CSV.")
    ...
    for a in agents:
        ...
        # Calculate final bonus after deductions
        deduction_percent = (a["mistakes_percent"] / 100) * (a["PreBonus"] + dist)
        deduction_value = a["mistakes_value"]
        a["FinalBonus"] = a["PreBonus"] + dist - deduction_percent - deduction_value
